...
@app.route('/add-demo-images')
def add_demo_images():
    product = Product.query.filter_by(name="Black Heavy Duty Tee").first()
    ...
    if existing_images:
        return "Images already added."

    image1 = ProductImage(product_id=product.id, url="images/product2.jpg")
    image2 = ProductImage(product_id=product.id, url="images/product2_alt1.jpg")
    image3 = ProductImage(product_id=product.id, url="images/product2_alt2.jpg")

    db.session.add_all([image1, image2, image3])
    db.session.commit()

    return "✅ Demo images added for Black Heavy Duty Tee."
